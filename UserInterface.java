 ...
 * 
 * @author <Hemanth S> and <2020501114>
 * 
	...
	**********************************************************************************************/
	...
	private void loadImageData() {
		try {
			...
			Scanner load = new Scanner(new File(str_FileName));
			int[][] l = new int[5][2];
	        int i = 0;
			while (load.hasNextLine()) {
				
				// Read in the line and trim the white space before and after the real data
				String inputLine = load.nextLine().trim();
				
				// Construct a scanner to parse each input line (should be two integer values/line)
				Scanner load_line = new Scanner(inputLine);
				
				// See if the first thing on the line is an int
				if (load_line.hasNextInt()) {
					int first = load_line.nextInt();
					l[i][0]=first;
				}
				if (load_line.hasNextInt()) {
					int second = load_line.nextInt();
					l[i][1]=second;
				}
				i++;
			}
			int bl= 100;
			Cell[][] obj1 = oddGameBoard.createBoard(bl, l);
			cell = obj1;
			for (int l2 = 0; l2<obj1.length; l2++) {
				for (int w = 0; w<obj1[0].length; w++) {
					if(obj1[l2][w].isStatus()==true) {
						Rectangle rect_angle = new Rectangle((l2*cellSize),(w*cellSize),cellSize,cellSize);
						rect_angle.setFill(Color.BLACK);
						oddCanvas.getChildren().add(rect_angle);
	...
	public void runSimulation(){
		...
		Cell [][] asus = game_Obj.generateNextGeneration(cell);
		if (toggle) {
			for (int l2 = 0; l2<asus.length; l2++) {
				for (int w = 0; w<asus[0].length; w++) {
					if(asus[l2][w].isStatus()==true) {
						Rectangle rect_angle = new Rectangle((l2*cellSize),(w*cellSize),cellSize,cellSize);
						rect_angle.setFill(Color.BLACK);
						evenCanvas.getChildren().add(rect_angle);
			...
			Cell [][] asus2 = game_Obj.generateNextGeneration(asus);
			cell = asus2;
			for (int l2 = 0; l2<asus2.length; l2++) {
				for (int w = 0; w<asus2[0].length; w++) {
					if(asus2[l2][w].isStatus()==true) {
						Rectangle rect_angle = new Rectangle((l2*cellSize),(w*cellSize),cellSize,cellSize);
						rect_angle.setFill(Color.BLACK);
						oddCanvas.getChildren().add(rect_angle);
